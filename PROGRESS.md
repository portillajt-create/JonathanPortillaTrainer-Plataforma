...
| Página | Qué hace |
|---|---|
| Gestión de Clientes | Alta/baja de clientes, suscripciones (Mensual/Trimestral/Semestral con vencimiento auto-calculado), alertas de vencimiento, eliminar cliente. Lista ordenada: activos primero, luego alfabético por nombre (ver §9) |
| Ficha del Atleta | Vista de solo lectura del onboarding + descarga en PDF con identidad de marca (ver §4 "PDFs"), con alerta roja si hay patologías/lesiones reales |
...
| Progreso | **Importador de CSV de Hevy** + **reporte de progreso en PDF** (con comentarios de tendencia) + tabla de ejercicios estancados + gráfica de 1RM estimado por ejercicio + gráficas de check-in |
...
11. Página nueva "Guías y Recursos", al final de la navegación de ambos roles (ver §4 para el detalle completo): política de datos + términos (movidos a `utils/legal.py`, ya no duplicados), videos guía (pendientes de subir a YouTube, ver §6), glosario de 10 conceptos que la plataforma ya usa.
12. Lluvia de ideas de 18 puntos (comparando con Trainerize/TrueCoach/Everfit/My PT Hub) revisada uno por uno con el usuario — veredictos completos en §9. De ahí salieron y se construyeron: plantillas de rutina reutilizables, historial de dietas/rutinas anteriores, ingresos estimados/mes, umbral "por vencer" de 5→2 días, y (**último cambio**) el cron de recordatorios automáticos (`scripts/recordatorios_diarios.py`) que reemplaza los dos avisos manuales anteriores — detalle completo en §9, probado con 10 escenarios simulados. **Pendiente para que funcione en producción:** agregar los 7 GitHub Secrets (§6, punto 1).
...
### PDFs con identidad de marca: ficha de onboarding y reporte de progreso (2026-09-25)
Pedido del usuario: el PDF de onboarding era "muy sencillo, no va con la estética de la página" → fondo negro, colores de la página (degradado cian-azul) y su logo; y un reporte igual de descargable en Progreso / Mi Progreso, con las mismas gráficas y comentarios de tendencia.

- **Tres módulos, cada uno con una sola responsabilidad**: `utils/pdf_base.py` (identidad visual: clase `PDFMarca` con fondo, portada con logo, secciones, tarjetas, KPIs, tabla, gráficas), `utils/pdf_export.py` (qué va en cada reporte), `utils/tendencias.py` (texto de los comentarios, lógica pura sin fpdf ni Streamlit).
- **Las gráficas se dibujan directo con primitivas de fpdf2** (líneas, círculos, polígonos con relleno translúcido), NO exportando las de Plotly a imagen. *Por qué:* exportar Plotly exige `kaleido`, que en sus versiones actuales necesita un Chrome instalado — Streamlit Community Cloud no lo garantiza; y matplotlib habría sumado una dependencia pesada con varias transitivas (la auditoría de seguridad fijó todas las versiones, cada dependencia nueva es superficie). Así no se agregó **ninguna** dependencia, y las gráficas quedan vectoriales y con la paleta de marca (cian `#5EEAD4`, azul `#3B82F6`, violeta `#A78BFA`), no la de Plotly.
- **Logo**: `assets/logo_full.png` en la portada, `assets/icon.png` en el encabezado de las páginas siguientes (ambos ya eran blancos sobre transparente — encajan directo sobre fondo negro).
- **Codificación `windows-1252`** (`core_fonts_encoding`) en vez del latin-1 por defecto de fpdf2: con Helvetica core trae además guion largo (–), comillas tipográficas, viñeta (•) y puntos suspensivos. Lo que no cabe (emoji que escriba un cliente en sus notas, flechas) se descarta en `_safe()` y se repega la puntuación que quede suelta ("hotel ." → "hotel.").
- **Gotcha de fpdf2 2.8.8 — `use_pattern()` (degradados)**: al salir del contexto re-emite el color de RELLENO usando el valor del color de LÍNEA, sin actualizar su registro interno. Efecto real que se vio: las tarjetas siguientes salían con el color del borde (más claras). Arreglo: todo uso de degradado va envuelto en `local_context()` (q/Q del PDF). Y para **texto** en degradado (títulos, igual que el `h1` de la app) hay que igualar color de texto y de relleno antes: `cell()` fuerza su propio color de texto si difiere del de relleno y tapa el degradado. Además `extend_before/after=True`, o la última letra del título ("g" de "Onboarding") se cortaba.
- **Botones de descarga con `data=` callable + `on_click="ignore"`**: el PDF se genera recién al hacer clic, no en cada carga de la página; y descargar no recarga la pantalla. OJO: Streamlit ejecuta ese callable **en otro hilo, sin contexto de sesión** — por eso `generar_pdf_*` reciben todos los datos ya cargados y NUNCA consultan Supabase ni usan Streamlit (en ese hilo no hay sesión). Para eso, `hevy_integration.py` ahora carga check-ins e historial **una sola vez** en `_render_pagina()` y los comparte entre las gráficas de pantalla y el PDF (antes cada sección los pedía por separado).
- **Reporte de progreso — contenido**: portada (cliente, objetivo del onboarding, rango de check-ins) → **Resumen** (viñetas: peso, adherencia promedio, bienestar, fuerza) → **Último check-in** (6 KPIs con cambio vs el check-in anterior, en cian si es favorable y rojo si no; + nota del cliente si la escribió) → **Check-ins semanales** (gráficas de peso, adherencia y bienestar, cada una con su comentario) → **Fuerza** (tabla "Ejercicios a tener en cuenta" si hay, y gráfica de 1RM estimado de los **4 ejercicios más entrenados en los últimos 3 meses**, sobre los últimos 6 meses, dos por fila). El botón aparece **solo en Progreso del admin, al final de la página** (mismo lugar que el de la ficha de onboarding) y no aparece si no hay ni check-ins ni historial. *Historia:* la primera versión (mismo día) lo ponía arriba de todo y también en Mi Progreso del cliente; el usuario pidió moverlo al final y que **ningún cliente pueda descargar ninguno de los dos reportes** — el entrenador decide si se lo comparte. Pie con aclaración de que los comentarios son reglas, no un diagnóstico.
- **Comentarios de tendencia (`tendencias.py`), mismo criterio que `analisis_progreso.py`: reglas simples y explicables, sin IA.** Escalas 1-10: promedio de las últimas k semanas vs las k anteriores (k ≤ 3), "subió/bajó" solo si la diferencia es ≥ 0.75 puntos, si no "estable"; se califica de favorable/desfavorable según la dirección buena de cada métrica (sueño y adherencia ↑, estrés y fatiga ↓) y se avisa si el último valor está en zona alta/baja. **Peso**: cambio total en kg y %, ritmo por semana, y aviso si los últimos 3 registros van al revés del total; solo se califica contra el **objetivo del onboarding** cuando es inequívoco (pérdida de grasa → bajar, hipertrofia → subir), si no queda neutral — el reporte lo lee también el cliente. **1RM**: promedio de las primeras 3 sesiones del periodo vs las últimas 3 (no primera vs última: en la prueba con datos reales, una sola sesión liviana hacía ver un "–28%" en Press de Banca), ±2% = estable, más la mejor marca y su fecha.
- Los umbrales de "zona alta/baja" (`UMBRAL_FATIGA_ALTA`, `UMBRAL_ESTRES_ALTO`, `UMBRAL_SUENO_BAJO`, `UMBRAL_ADHERENCIA_DIETA_BAJA`) **se movieron de `checkin.py` a `tendencias.py`**, y `checkin.py` los importa de ahí: las alertas de deload/adherencia del admin y los comentarios del PDF usan exactamente los mismos números.
- **Probado**: PDFs generados con 8 check-ins simulados + las **4.993 filas reales** del CSV de Hevy del cliente de prueba, y los casos borde (1 solo check-in, sin historial de Hevy, sin ningún dato), revisados página por página convertidos a imagen. Y en la app (arnés con datos simulados, sin tocar Supabase): las 3 vistas (Mi Progreso, Progreso admin, Ficha del Atleta) sin errores, y al hacer clic el servidor devuelve el PDF (`application/pdf`, ~175 KB), interceptando la descarga en el navegador para no guardar archivos.
- **Para ver un PDF como imagen en local**: el Python del sistema (no el venv del proyecto) tiene PyMuPDF — `python -c "import pymupdf; ..."` con `page.get_pixmap()`. El venv no lo necesita.
...
- **`st.container(key="x")` genera la clase CSS `st-key-x`** → así se aplica CSS a un widget puntual sin afectar toda la app. Ej: las pestañas de "Check-in Semanal" van envueltas en `st.container(key="checkin_semanas")` para que el CSS de `theme.py` que las resalta (degradado en la activa, atenuada la inactiva) NO toque las otras `st.tabs` del proyecto (login en `app.py`).
- **Las `st.tabs` de streamlit 1.62 son `react-aria`, no BaseWeb.** Cada pestaña es `<div data-testid="stTab" role="tab" aria-selected="true|false">`, el subrayado deslizante es `.react-aria-SelectionIndicator`. NO existe `[data-baseweb="tab"]` (eso era streamlit viejo). Para estilos activa/inactiva sin que se inviertan al cambiar de pestaña: usar `[aria-selected="true"]` y `[aria-selected="false"]` como pares mutuamente excluyentes (misma especificidad), **no** una regla base + override. Y **`transition` en `opacity` sobre `[data-testid="stTab"]` se queda pegada en un valor intermedio** al cambiar de pestaña (streamlit rehace algo en el DOM y `getComputedStyle` devuelve la opacity a medio animar) — se puso `transition: none !important` para esas pestañas.
...
| `modules/checkin.py` | 316 | Check-in semanal, alertas de adherencia y deload. Lógica de semanas importada de `utils/semanas_checkin.py`; umbrales de zona alta/baja importados de `utils/tendencias.py` |
| `scripts/recordatorios_diarios.py` | 213 | **Cron standalone** (no importa Streamlit) de recordatorios automáticos de check-in y vencimiento, disparado por `.github/workflows/recordatorios.yml`. Usa `service_role`, ver §9 |
| `app.py` | 352 | Login, recuperación de contraseña, roles, navegación, selector de cliente, bloqueo por suscripción vencida |
| `modules/nutricion.py` | 365 | Calculadora TDEE/macros, planificador de dieta, historial de dietas anteriores |
| `utils/queries.py` | 388 | **Único punto de acceso a la BD.** Ojo con la paginación en `list_historial_entrenamientos`. `list_clientes_con_suscripcion()` devuelve los clientes ya ordenados (activos primero, luego alfabético) |
| `utils/theme.py` | 291 | CSS de identidad visual, incluidos los estilos `st-key-*` |
| `modules/admin_clientes.py` | 250 | Gestión de clientes y suscripciones |
| `modules/hevy_integration.py` | 348 | Página Progreso: importador de CSV, botón del reporte PDF, tabla de estancados, gráfica de 1RM, gráficas de check-in (6 métricas: peso, adherencia dieta/entreno, sueño, estrés, fatiga; + nota libre del cliente si la escribió). Check-ins e historial se cargan una sola vez en `_render_pagina()` |
| `modules/onboarding.py` | 258 | Formulario del cliente + ficha del admin (con botón de PDF) |
...
| `utils/pdf_export.py` | 359 | Contenido de los dos PDFs: ficha de onboarding y reporte de progreso. Sin Streamlit ni Supabase (corre en otro hilo al hacer clic) |
| `utils/pdf_base.py` | 537 | Identidad visual de los PDFs (clase `PDFMarca`): fondo negro, logo, degradado cian-azul, tarjetas, KPIs, tabla y gráficas de líneas dibujadas con fpdf2 |
| `utils/tendencias.py` | 329 | Comentarios automáticos de tendencia del reporte de progreso (reglas, sin IA) + umbrales de zona alta/baja compartidos con `checkin.py` (sin Streamlit) |
...
| `utils/semanas_checkin.py` | 75 | Lógica pura (sin Streamlit) de semanas de check-in — `semana_a_reportar()`, `semana_en_curso_reportable()`, `rango_semana()`. Extraída de `checkin.py` para poder importarla desde `scripts/recordatorios_diarios.py` |

**Archivos de configuración:** `.streamlit/config.toml` (tema oscuro), `.env.example`, `requirements.txt` + `requirements-lock.txt` (versiones fijadas), `scripts/requirements-recordatorios.txt` (dependencias mínimas del cron), `.github/workflows/keep-alive.yml`, `.github/workflows/recordatorios.yml`, `.github/dependabot.yml`.
...
- **Vista de calendario (D.16)** — decidido "ambos" (cliente y admin), diseño ya pensado (2026-09-08) pero el usuario decidió no ejecutarlo por ahora: "ya te entendí la idea pero no considero ejecutarla ya". Datos que usaría (ya existen, sin SQL nuevo): vencimiento de suscripción (`fecha_vencimiento` de `vista_suscripciones`) + estado de cada semana de check-in (`utils/semanas_checkin.py`). Se propuso página nueva "Mi Calendario"/"Calendario" con `streamlit-calendar` (librería de terceros), lógica en un módulo nuevo sin Streamlit (`utils/calendario.py`, mismo patrón que `semanas_checkin.py`). Quedan 2 decisiones abiertas para cuando se retome: (1) del lado admin, ¿por cliente seleccionado (como el resto de páginas admin, recomendado) o vista agregada de todos los clientes a la vez (más potente, más riesgo de saturarse visualmente)? (2) ¿esos 2 eventos bastan o se agrega algo más (ej. día en que se actualizó la dieta/rutina)?

### Aprobados, en curso o hechos
- **Plantillas de rutina reutilizables — HECHO (2026-09-08).** "Me encanta, ejecútalo". Tabla nueva `plantillas_rutina` (id, nombre, bloques JSONB, creado_por), RLS exclusivo-admin (mismo patrón que `alertas_descartadas`). En `render_admin` de Entrenamiento, un expander "🗂️ Plantillas de rutina" con: guardar los ejercicios que se están armando con un nombre; cargar una guardada (reemplaza TODO el editor, mismo comportamiento que "Generar ejemplo de rutina" — sin diálogo de confirmación, porque nada toca la rutina activa hasta "Guardar y notificar"); borrar. Probado end-to-end con datos simulados: se guardó una plantilla en un cliente y se cargó exitosamente en otro, con todos los campos del ejercicio intactos. **SQL corrido y confirmado por el usuario (2026-09-08).**
...
- **Check-in de la semana pasada**: cron corre lunes/miércoles/viernes (`DIAS_CHECKIN = {0, 2, 4}`); revisa cada cliente activo con la misma `semana_a_reportar()` de siempre (extraída a `utils/semanas_checkin.py`, sin Streamlit, para poder importarla desde el cron); si sigue sin reportarse, avisa — hasta 3 veces por semana mientras siga pendiente. **La ventana de la semana en curso (habilitada desde el jueves) no cambia** — el cron nunca avisa de eso, solo de la semana ya cerrada; sigue siendo una opción que el cliente usa por su cuenta si quiere adelantarla.
- **Vencimiento**: se envía **una sola vez** por ciclo, el día en que faltan ≤2 días — mismo umbral que el badge visual (ver abajo, ya son un único umbral). Esta parte del cron corre **todos los días** (no solo L/M/V), para no depender de que el vencimiento caiga justo en uno de esos 3 días. La deduplicación compara `created_at` de la última `alerta_vencimiento` contra `fecha_ultimo_pago` de `vista_suscripciones`: si el cliente renueva, un aviso viejo de un ciclo anterior no bloquea el aviso del ciclo nuevo.
- **Arquitectura**: `scripts/recordatorios_diarios.py`, script standalone (no importa Streamlit ni `config.py`) disparado por `.github/workflows/recordatorios.yml` (cron diario 13:00 UTC = 8am Bogotá + `workflow_dispatch` para probarlo a mano), usando la clave `service_role` de Supabase **solo ahí, nunca en la app ni en el repo** — necesaria porque este script escribe notificaciones directo a la tabla (sin `auth.uid()`, a diferencia de `crear_notificacion_sistema`) y lee `vista_suscripciones` de todos los clientes a la vez. Se apoya en esa vista (no recalcula fechas a mano) para que el badge que ve el admin y el criterio del cron sean siempre la misma fuente de verdad. Dependencias propias en `scripts/requirements-recordatorios.txt` (solo `supabase`, sin Streamlit/pandas/plotly) para que el Action instale rápido.
- **Probado exhaustivamente** con un mock del cliente Supabase en memoria (sin tocar la BD real), cubriendo 9 escenarios: cliente sin reportar (sí avisa), ya reportó (no avisa), vencido (no avisa nada), registrado después de la semana en cuestión (no avisa), por vencer sin aviso previo (sí avisa), por vencer ya avisado este ciclo (no repite), renovó después de un aviso viejo (sí avisa, el viejo no bloquea), vencido nunca recibe "por vencer", sin correo (no avisa). Más un décimo escenario: día que no es L/M/V, sin nadie por vencer → cero notificaciones creadas.
- **GitHub Secrets configurados y verificados en producción (2026-09-08).** El usuario agregó los 6 secrets (`SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASSWORD` — `SMTP_FROM` se dejó sin crear a propósito, el script usa `SMTP_USER` como default). Se disparó el workflow manualmente (`workflow_dispatch`) contra producción real: conectó bien a Supabase (9 clientes reales leídos), omitió el aviso de check-in por no ser lunes/miércoles/viernes (tal como se diseñó) y revisó vencimientos sin encontrar candidatos (0 avisados, cero correos reales enviados en esta corrida de prueba). Corre solo, sin intervención, todos los días a las 8am Bogotá.
- **Feedback real de un cliente (2026-09-09)**: le llegó el correo de check-in faltante (confirma que el cron ya funciona en la práctica, no solo en la prueba manual) y sugirió agregar el link de la plataforma, porque el correo solo traía texto sin forma de entrar directo.
- **Link de plataforma: solo donde tiene sentido entrar a la app (decisión del usuario, 2026-09-09)**. `URL_PLATAFORMA` (definida en `config.py`, con copia standalone en `scripts/recordatorios_diarios.py` porque ese script no puede importar `config.py`) se agrega al final del mensaje SOLO en: `checkin_faltante` (cron) y `dieta_actualizada`/`rutina_actualizada` (`nutricion.py`/`rutinas.py`) — en los tres, la acción que se le pide al cliente es entrar a la plataforma. **NO se agrega en `alerta_vencimiento`**: ahí lo que se le pide es contactar al entrenador, no entrar a la app, así que el link no aplica — decisión explícita del usuario tras verlo, no un olvido. Como el correo se manda en texto plano (`MIMEText` sin subtype), se confía en que los clientes de correo (Gmail, Outlook, Apple Mail...) auto-detectan la URL como link, no hizo falta pasar a HTML.
- **"Ejercicios a tener en cuenta" (estancamiento): revertido a visible para los dos roles (2026-09-09)**. Se había ocultado del cliente pensando que aplicaba el mismo criterio de deload/adherencia (evitar que una alerta le llegue sin que el entrenador medie) — pero el usuario aclaró que esa preocupación era específicamente por el **correo**, y esta tabla nunca lo dispara (es un `st.dataframe` puro, sin botón). Al ser solo visual, el usuario prefiere que el cliente también la vea en su propio "Mi Progreso". Se quitó el parámetro `mostrar_estancamiento` de `_render_historial_ejercicio()`, vuelve a llamarse igual desde `render_admin` y `render_cliente`.
- **Orden de clientes en TODA la app (pedido del usuario, 2026-09-19)**: dos criterios, en este orden — (1) activos primero, (2) alfabético por nombre. Vive en un solo lugar: `_clave_orden_cliente()` en `utils/queries.py`, aplicado dentro de `list_clientes_con_suscripcion()`, que por lo tanto **ya devuelve la lista ordenada**. La consumen la lista de Gestión de Clientes y el **selector de cliente compartido** de las páginas admin (`_selector_cliente()` en `app.py`: Ficha, Nutrición, Entrenamiento, Progreso), que pasó de `list_clientes()` (solo ordenaba por nombre en la BD, sin conocer el estado) a `list_clientes_con_suscripcion()`. Efecto colateral bueno: el cliente preseleccionado al entrar a esas páginas ahora es un activo (antes era el primero alfabético, aunque estuviera inactivo). Las alertas de deload/adherencia (`checkin.py`) también iteran esta lista, así que salen en el mismo orden. Un primer intento (2026-09-19, mismo día) puso el orden solo en `admin_clientes.py`; se movió a `queries.py` para no duplicar la lógica. "Activo" = mismo criterio de la métrica "Activos" (estado Activo y no vencido), o sea que **"Por vencer" cuenta como activo** (todavía tiene acceso); Inactivos, Vencidos y Sin suscripción van juntos después. El nombre se compara **sin tildes ni mayúsculas** (`unicodedata`): con el orden por defecto de Python, "Álvaro" quedaba después de "Zoe" y "dayana" después de "Xavier". Si el cliente no tiene nombre, ordena por su correo. Probado con los 9 clientes reales de la captura del usuario + casos borde.
- **Ajustes visuales (2026-09-10)**:
  - **Pestañas de "Check-in Semanal" más llamativas**. "Semana pasada" / "Semana en curso" eran texto plano con el subrayado nativo — fácil que el cliente llenara la semana equivocada. Ahora: envueltas en `st.container(key="checkin_semanas")`, con CSS en `theme.py` que le da a la ACTIVA el degradado cian-azul de la marca (mismo de títulos y botones primary) + texto oscuro/negrita + sombra, y deja la INACTIVA atenuada (opacity 0.5). Las dos ocupan todo el ancho, con el texto que baja de línea en celular en vez de desbordar. Verificado en 375px y en desktop, y que al cambiar de pestaña el estilo no se invierte. Ver §4 "Patrones de Streamlit" para los detalles de `react-aria` / `transition`.
  - **Quitado el banner azul de Entrenamiento** que decía "Los ejercicios que un cliente sigue entrenando pero sin progreso real de fuerza se listan en Progreso" — el usuario lo pidió: es redundante, él ya sabe que eso vive en Progreso. `render_alertas_entrenamiento()` ahora solo muestra la alerta de deload; el comentario con el plan de traer la tabla de estancamiento acá se conserva.
- **Confirmado: estancamiento nunca envía correo** — `_render_ejercicios_a_revisar()` es un `st.dataframe` puro, sin botón ni llamado a `crear_notificacion` en ningún lado del código.
- **Deload deja de poder notificar al cliente (decisión del usuario, 2026-09-09)**: el botón "🔔 Sugerir" de `render_alertas_deload()` (Entrenamiento) se quita por completo — quedó igual de informativo que estancamiento, solo un `st.warning()` por cliente, sin ninguna forma de avisarle (ni in-app ni correo). Antes era opt-in (solo pasaba si el admin le daba clic a un cliente puntual), pero el usuario decidió que ni así: esa conversación la maneja directamente, fuera de la plataforma.
- **Mecanismos manuales viejos retirados por duplicar al cron (2026-09-09)**: al construir B.7 (recordatorios automáticos) no se habían quitado sus predecesores, y quedaron corriendo en paralelo con riesgo real de avisar dos veces al mismo cliente:
  - `_generar_notificacion_checkin_faltante()` en `checkin.py` — se disparaba "al vuelo" cada vez que el cliente entraba a "Mis Notificaciones". Eliminada por completo; `scripts/recordatorios_diarios.py` ya cubre exactamente el mismo caso, automático, lunes/miércoles/viernes.
  - Botón "🔔 Recordar" de vencimiento en `admin_clientes.py` (`_render_alertas_vencimiento`) — ahora es puramente informativo (mismo criterio que deload/estancamiento), porque el cron ya avisa solo, una única vez por ciclo, cuando faltan ≤2 días.
  - Limpieza de imports que quedaron sin uso en ambos módulos (`crear_notificacion`, `crear_notificacion_sistema`, `current_cliente_id`, `fecha_bogota`, `hoy_bogota`, `get_cliente` — cada uno solo donde de verdad dejó de usarse).
