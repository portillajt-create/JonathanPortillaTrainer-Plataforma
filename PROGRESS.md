...
| `modules/admin_clientes.py` | 267 | Gestión de clientes y suscripciones |
...
- **Orden de la lista en Gestión de Clientes (pedido del usuario, 2026-09-19)**: dos criterios, en este orden — (1) activos primero, (2) alfabético por nombre. Implementado como `_clave_orden()` en `admin_clientes.py`, aplicado con `sorted()` al renderizar (no toca `list_clientes_con_suscripcion`, así que los selectores de cliente de las otras páginas admin siguen igual). "Activo" = mismo criterio de la métrica "Activos" (estado Activo y no vencido), o sea que **"Por vencer" cuenta como activo** (todavía tiene acceso); Inactivos, Vencidos y Sin suscripción van juntos después. El nombre se compara **sin tildes ni mayúsculas** (`unicodedata`): con el orden por defecto de Python, "Álvaro" quedaba después de "Zoe" y "dayana" después de "Xavier". Si el cliente no tiene nombre, ordena por su correo. Probado con los 9 clientes reales de la captura del usuario + casos borde.
