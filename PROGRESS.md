...
- **Hallazgo y arreglo — "Ejercicios a tener en cuenta" se le mostraba al cliente (2026-09-09)**: al revisar el pedido de que estancamiento/deload nunca notifiquen al cliente, se encontró que `_render_historial_ejercicio()` (Progreso) llamaba a `_render_ejercicios_a_revisar()` (la tabla de estancamiento) tanto en `render_admin` como en `render_cliente` — el cliente veía en su propio "Mi Progreso" la tabla que dice qué ejercicios no muestran progreso real, justo lo que el criterio ya documentado (§6, punto 2) quería evitar ("decirle 'llevas semanas sin progresar' desmotiva"). Se agregó el parámetro `mostrar_estancamiento` (`True` solo en `render_admin`) para que la tabla quede exclusiva del admin; el gráfico de "Progreso por ejercicio" de la misma sección sigue igual para los dos roles, no se tocó.
