...
- **Videos alojados en YouTube como "no listado"**, decisión del usuario tras comparar opciones. Los archivos (`GUIA 1.mp4` y `GUIA 2.mp4`, en la carpeta del proyecto fuera de `jp_trainer_dashboard/`) pesan 32 y 34 MB. *Por qué no van al repo:* un binario de ese tamaño en git infla el historial para siempre, incluso si se borra después. *Por qué no van a Supabase Storage:* el plan gratuito tiene un tope de banda mensual ajustado, y varios clientes viendo 30+ MB cada uno lo agotaría rápido. YouTube no listado es gratis, sin límite real de banda, y solo lo ve quien tenga el link.
  - **"Cómo usar la plataforma" (GUIA 2) ya está cargado** (2026-09-05): `https://www.youtube.com/watch?v=zoYnKWl_TpA` — verificado en YouTube Studio como "No listado". YouTube lo clasificó como Short (formato vertical, 1:20); no afecta a `st.video()`, que lo embebe igual. **Sigue pendiente "Cómo exportar tu historial de Hevy" (GUIA 1)**, `url=None` en `recursos.py:VIDEOS_GUIA` — mientras tanto esa tarjeta muestra "🎬 Video en camino" en vez de romper. Ojo: en el canal quedó también un Short **sin publicar** (título "¿Cómo usar", sin descripción, 1:12) — parece un intento anterior a medias; no se tocó, el usuario decide si lo borra o lo termina.
...
1. **Subir `GUIA 1.mp4` (exportar historial de Hevy) a YouTube como "no listado" y pasar el link** — el otro video (usar la plataforma) ya está cargado (ver §4). Mientras tanto esa tarjeta no rompe, solo avisa "video en camino". También vale la pena que el usuario revise el glosario (`modules/recursos.py:GLOSARIO`) y ajuste lo que no calce con cómo él mismo explica esos conceptos.
