...
- **Feedback real de un cliente (2026-09-09)**: le llegó el correo de check-in faltante (confirma que el cron ya funciona en la práctica, no solo en la prueba manual) y sugirió agregar el link de la plataforma, porque el correo solo traía texto sin forma de entrar directo. Se agregó `URL_PLATAFORMA` en `scripts/recordatorios_diarios.py`, al final de los dos mensajes (checkin_faltante y alerta_vencimiento) — como el correo se manda en texto plano (`MIMEText` sin subtype), se confía en que los clientes de correo (Gmail, Outlook, Apple Mail...) auto-detectan la URL como link, no hizo falta pasar a HTML.
- **Mismo link agregado a dieta/rutina actualizada (2026-09-09)**: `URL_PLATAFORMA` se agregó también en `config.py` (única fuente para toda la app, aparte de la copia standalone del cron) y se usa en los mensajes de `nutricion.py`/`rutinas.py` que avisan "Tu plan nutricional fue actualizado"/"Tienes una rutina nueva o actualizada".
